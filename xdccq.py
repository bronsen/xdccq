...
def print_success(string):
    pcolor(string, 'dkgreen')


def echo(string):
    pcolor(string, 'ltblue')

print_info = print_success

cmd_help = {}
gen_help = [
    "\037[help, ?]\037 <cmd>    - prints this help; or help for 'cmd'\n",
...
cmd_help['?'] = """\n\002/xdccq\002 \037[help, ?]\037 <cmd>\n   \00303By \
        itself, help/? prints out the available commands along withshort \
        descriptions of each.  If you supply a command \002cmd\002, a longer \
        help description of that command and its usage is given.\003"""
cmd_help['ls'] = """\n\002/xdccq\002 \037[ls, list]\037\n   \00303Lists the \
        file gets currently in the queue.\00303"""
cmd_help['get'] = """\n\002/xdccq\002 \037get\037 [bot] [#, #-#, #-#%#]\n   \
        \00303Adds commands to the local queue "/ctcp \037bot\037 xdcc send \
        \037#\037".  A number ["7"], a number range ["1-5"], a range with \
        step ["0-10%2"] (1, 3, 5, ...), or a comma separated list ["7, 8-11, \
        15, 16-25%3"] may be given.\003"""
cmd_help['rm'] = """\n\003/xdccq\002 \037rm\037 [bot] <#, #-#>\n   \
        \00303Removes commands from local queue.  If only \037bot\037 is \
        supplied, it removes all commands dealing with \037bot\037.  If it is \
        supplied with a number or number range, any commands in that range \
        are removed.   If you had numbers "8, 9, 11, 12, 14" queued, and \
        removed "8-14", it would remove all of those package numbers for that \
        bot without resulting in an error for the missing "10" and "13".\003\
        \n   \00303New in 0.3: rm will now also call 'cancel' if the active \
        transfer is from [bot] and the optional range argument was not given.\
        \003"""
cmd_help['cancel'] = """\n\003/xdccq\002 \037cancel\037\n   \00303If a DCC \
        handled by xdccq is currently active, it cancels that transfer and \
        starts the next one on the queue.  If the 'active' transfer is \
        remotely queued, xdccq attempts to unqueue it by issuing a '/msg \
        <bot> xdcc remove' command.  If this command fails, this transfer \
        will continue to remotely pend; take notice!  In multiq mode, this \
        cancels \037all\037 transfering packs.\003"""
cmd_help['multiq'] = """\n\003/xdccq\002 \037multiq\037\n   \00303The default \
        mode for xdccq is to operate with one global queue and dish out 1 \
        pack request at a time.  This toggles 'Multi-queue' mode, which keeps \
        track of 1 transfer per bot in the queue.\003"""
...
class cmdQueue(Queue):
    ...
    def getBotPackSet(self, botname, r=[]):
        ...
        packs = [cmd for cmd in self.data
                 if cmd.bot.lower() == botname.lower()]
        # if supplied a range, cut list to those in the range
        if r:
            packs = [cmd for cmd in packs if cmd.num in r]
        return packs
...
def ls(a):
    ...
    for cmd in Active.transfering():
        ...
        s.append("Pack #%d from %s on %s@%s being transfered at %s." %
                 (cmd.num, cmd.bot, cmd.channel, cmd.network, cps))
    for cmd in Active.queued():
        s.append("Pack #%d from %s on %s@%s queued remotely [position %d]." %
                 (cmd.num, cmd.bot, cmd.channel, cmd.network,
                  cmd.queue_position))
...
def get(a):
    if len(a) != 4:
        print_error("Error: invalid arguments for get.  \
                    /xdccq get [bot] [#, #-#]")
        return
    bot = str(a[2])
    try:
...
def rm(a):
    global CommandQueue, Active
    if len(a) not in (3, 4):
        print_error("Error: invalid arguments for 'rm'.  \
                    /xdccq rm [bot] <#, #-#>")
        return
    items_to_delete = []
    if len(a) == 4:
        try:
            items_to_delete = numToList(a[3])
        except ValueError, exc:
            # exc.args[0] is in the set ['number', 'range']
            print_error("Error: %s contains invalid %s." % (a[3], exc.args[0]))
            return
...
USAGE_STR = "Usage: \002/xdccq\002 [cmd] [args], \002/xdccq\002 \037help\037 \
        for commands."
...
def transferCheck(data):
    """This watchdog function runs as long as we are transferring something.
    It goes through the active queue looking for packs that are neither
    transferring nor queued. If it finds one as such, it marks it as 'dead'.
    If it stays 'dead' until the next run, it will retry the request. If the
    retry threshold (3) is met, the command will be removed."""
    global Active
    ...
    for cmd in Active:
        ...
        if not cmd.dead:
            ...
            print_error("Previous attempt to get file (#%d from %s) failed. \
                        Repeating (%d of 3 retries)" %
                        (cmd.num, cmd.bot, cmd.retries))
        else:
            print_info(
                "At this point we'd want to remove (#%d from %s, %d)." %
                (cmd.num, cmd.bot, cmd.retries))
            # run()
    return True
...
def run():
    ...
    run just about any time there is an interaction with the queue (get,
    delete, dcc events, etc)."""
    ...
    ** Closing Connection: DCC Timeout (180 Sec Timeout)
    ...
    for cmd in Active:
        ...
        if 'queue' in message.lower():
            cmd.queued = True
            print_info("xdccq detected the active file being placed on a \
                       remote queue")
        ...
        if res:
            try:
                ...
                print_error("an error occured parsing the remote queue \
                            position")


def dccComplete(split, full, data):
    # ['just4u.txt', '/home/jmoiron/.xchat2/downloads/just4u.txt.1',
    # 'just|4|u', '38373']
    if not Active:
        return
    ...
    for cmd in Active.transfering():
        if cmd.bot == str(split[2]):
            ...
            print_info("Received file \"%s\" from %s on %s@%s." %
                       (cmd.file, cmd.bot, cmd.channel, cmd.network))
            Active.remove(cmd)
    run()
...
def dccStall(split, full, data):
    ...
    botname = str(split[2])
    for cmd in (c for c in Active if c.bot == botname):
        print_error("Requested file \"%s\" has stalled during transport." %
                    (split[1]))
        ret = cmd.retry()
        if ret:
            print_info("Re-requesting file \"%s\" (%d of 3 retries)" %
                       (split[1], Active.retries))
        else:
            print_error(
                "Retry limit reached for file \"%s\". Stopping the queue." %
                (split[1]))
...
dccRecvCompleteHook = xchat.hook_print("DCC RECV Complete", dccComplete,
                                       "data")
